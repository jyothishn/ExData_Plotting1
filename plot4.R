...
## plots are similar to the 3 previous assignments
PlotDataFor4 <- function(data) {
  par(mfrow=c(2, 2))
  with(data, {
    ...
    vars <- paste("Sub_metering_", 1:3, sep="")
    for (i in seq_along(vars)) {
      var <- vars[i]
      x <- data[[var]]
      lines(datetime, x, col=cols[i])
    }
    
    legend("topright", bty="n", legend=vars, col=cols,lty="solid")
